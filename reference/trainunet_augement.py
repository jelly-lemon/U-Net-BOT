...
def main():
    ...
    seed = 1
    image_datagen.fit(x_train_pr, augment=True, seed=seed)
    mask_datagen.fit(y_train_pr, augment=True, seed=seed)
    image_generator = image_datagen.flow(x_train_pr,batch_size=args.batchsize,,seed=seed)
    mask_generator = mask_datagen.flow(y_train_pr,batch_size=args.batchsize,,seed=seed)
    ...
    # )
    # seed = 2
    # val_image_datagen.fit(x_val_pr, augment=True, seed=seed)
    # val_mask_datagen.fit(y_val_pr, augment=True, seed=seed)
    # val_image_generator = val_image_datagen.flow(x_val_pr,batch_size=args.batchsize)
    # val_mask_generator = val_mask_datagen.flow(y_val_pr,batch_size=args.batchsize)
    ...
    history = model.fit_generator(
        ...
        steps_per_epoch = (len(x_train_pr)//args.batchsize)*10,
        # steps_per_epoch = (len(x_train_pr)//args.batchsize),
        callbacks=[checkpoint],
        epochs=args.epoch,
        # validation_data=val_generator,
        validation_data=(x_val_pr,y_val_pr),
        # validation_steps= (len(x_val_pr)//args.batchsize)*10,
        verbose=1,
        shuffle=True
    )
