...
def main():
    ...
    # define model
    base_model = Unet(BACKBONE, input_shape=(512, 512, 1), encoder_weights=None)

    # multi gpu
    par_model = multi_gpu_model(base_model, gpus=args.gpu_num)
    par_model.compile('Adam', loss=args.loss, metrics=['dice_score','iou_score','jaccard_score','f1_score','f2_score'])
    par_model.summary() 
    ...
    checkpoint = AltModelCheckpoint(file_path, base_model, monitor='val_score', verbose=1, save_best_only=True, mode='max')

    history = par_model.fit(
        x=x_train_pr,
        y=y_train_pr,
